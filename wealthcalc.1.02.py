...
def printWealthMatrix(): 
    ...
    for x in range (0, maxyears):
        if wtotal[x] > 0:
            ...
            print("Total: " + str(format(wtotal[x], ',.2f') + "\n"))
...
def shouldInvest(x):
    if wtotal[x] > 0 and coh[x] / wtotal[x] >= .15 and coh[x] * icohratio > mininvestment:
        return True
    else:
        return False
